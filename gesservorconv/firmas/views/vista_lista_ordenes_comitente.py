...
class VistaListaOrdenesComitente(
    MixinAccesoRequerido,
    MixinPermisoRequerido,
    UserPassesTestMixin,
    TemplateView
    ...
    paginate_by: int = 10
    template_name: str = "firmas/listar_ordenes_comitente.html"
    ...
    def get_queryset(self: Self) -> QuerySet[OrdenServicio]:
        '''
        Se filtra las órdenes que pertenezcan al comitente
        '''
        ordenes: QuerySet[OrdenServicio] = \
            OrdenServicio.objects.exclude(
                pk__in=Servicio.objects.filter(
                    orden_servicio__isnull=False
                ).values_list(
                    "orden_servicio", flat=True
                )
            ).filter(
                ...
                debe_firmar=~Q(
                    pk__in=(
                        FirmaOrden.objects.filter(
                            Q(usuario_firmante=self.request.user) &
                            Q(tiempo_firma__isnull=False) &
                            ~Q(documento_firmado="")
                        ).values_list(
                            "orden_firmada__solicitud_servicio"
                        )
                    )
                )
            ).annotate(
    ...
    def get_context_data(
        ...
        **kwargs: Dict[str, Any]
        ...
        ordenes: QuerySet[OrdenServicio] = self.get_queryset()
        paginador_curso: Paginator = Paginator(
            ordenes.filter(
                Q(
                    cancelacion_orden__isnull=True
                ) &
                Q(
        ...
        paginador_cancelado: Paginator = Paginator(
            ordenes.filter(
                Q(
                    cancelacion_orden__isnull=False
                )
            ),
            self.paginate_by
        )
    ...
    def get(self: Self, request: HtmxHttpRequest) -> HttpResponse:
        if request.htmx:
            ...
            if si_nombre and nombre.strip() != "":
                ...
                ordenes = ordenes.filter(busqueda)
            tipo: Optional[str] = request.GET.get("tipo")
            if not tipo or tipo == "curso":
                ordenes_curso: QuerySet[OrdenServicio] = ordenes.filter(
                    Q(
                        cancelacion_orden__isnull=True
                    ) &
                    Q(
                ...
                if tipo == "curso":
                    return render(
                        request,
                        "parciales/_lista_ordenes_en_curso_comitente.html",
                        {"pagina_curso": objeto_pagina_curso}
                    )
            if not tipo or tipo == "suspendido":
                ordenes_suspendido: QuerySet[OrdenServicio] = ordenes.filter(
                    Q(
                        cancelacion_orden__isnull=True
                    ) &
                    Q(
                ...
                if tipo == "suspendido":
                    return render(
                        request,
                        "parciales/_lista_ordenes_suspendidas_comitente.html",
                        {"pagina_suspendido": objeto_pagina_suspendido}
                    )
            if not tipo or tipo == "cancelado":
                ordenes_cancelado: QuerySet[OrdenServicio] = ordenes.filter(
                    Q(
                        cancelacion_orden__isnull=False
                    )
                )
                ...
                if tipo == "cancelado":
                    return render(
                        request,
                        "parciales/_lista_ordenes_canceladas_comitente.html",
                        {"pagina_cancelado": objeto_pagina_cancelado}
                    )
            return render(
                request,
                "parciales/_lista_ordenes_comitente.html",
                {
                    "pagina_curso": objeto_pagina_curso,
                    "pagina_suspendido": objeto_pagina_suspendido,
                    "pagina_cancelado": objeto_pagina_cancelado

...
from django.db.models import (
    Case, CharField, F, Min, Q, Value, When
)
...
class VistaListaConveniosResponsable(
    MixinAccesoRequerido,
    MixinPermisoRequerido,
    UserPassesTestMixin,
    TemplateView
    ...
    paginate_by: int = 10
    template_name: str = "firmas/listar_convenios_responsable.html"
    ...
    def get_queryset(self: Self) -> QuerySet[Convenio]:
        '''
        Se filtra los convenios que pertenezcan al responsable técnico
        '''
        convenios: QuerySet[Convenio] = \
            Convenio.objects.exclude(
                pk__in=Servicio.objects.filter(
                    convenio__isnull=False
                ).values_list(
                    "convenio__solicitud_servicio_id", flat=True
                )
            ).filter(
                ...
                tiempo_creacion=Min(
                    "solicitud_servicio__comitentesolicitud__tiempo_decision",
                    filter=Q(
                        solicitud_servicio__comitentesolicitud__tiempo_decision__isnull=False
                    )
                )
            ).annotate(
    ...
    def get_context_data(
        ...
        **kwargs: Dict[str, Any]
        ...
        convenios: QuerySet[Convenio] = self.get_queryset()
        paginador_curso: Paginator = Paginator(
            convenios.filter(
                Q(
                    cancelacion_convenio__isnull=True
                ) &
                Q(
        ...
        paginador_cancelado: Paginator = Paginator(
            convenios.filter(
                Q(
                    cancelacion_convenio__isnull=False
                )
            ),
            self.paginate_by
        )
    ...
    def get(self: Self, request: HtmxHttpRequest) -> HttpResponse:
        if request.htmx:
            ...
            if si_nombre and nombre.strip() != "":
                ...
                convenios = convenios.filter(busqueda)
            tipo: Optional[str] = request.GET.get("tipo")
            if not tipo or tipo == "curso":
                convenios_curso: QuerySet[Convenio] = convenios.filter(
                    Q(
                        cancelacion_convenio__isnull=True
                    ) &
                    Q(
                ...
                if tipo == "curso":
                    return render(
                        request,
                        "parciales/_lista_convenios_en_curso_responsable.html",
                        {"pagina_curso": objeto_pagina_curso}
                    )
            if not tipo or tipo == "suspendido":
                convenios_suspendido: QuerySet[Convenio] = convenios.filter(
                    Q(
                        cancelacion_convenio__isnull=True
                    ) &
                    Q(
                ...
                if tipo == "suspendido":
                    return render(
                        request,
                        "parciales/_lista_convenios_suspendidos_responsable.html",
                        {"pagina_suspendido": objeto_pagina_suspendido}
                    )
            if not tipo or tipo == "cancelado":
                convenios_cancelado: QuerySet[Convenio] = convenios.filter(
                    Q(
                        cancelacion_convenio__isnull=False
                    )
                )
                ...
                if tipo == "cancelado":
                    return render(
                        request,
                        "parciales/_lista_convenios_cancelados_responsable.html",
                        {"pagina_cancelado": objeto_pagina_cancelado}
                    )
            return render(
                request,
                "parciales/_lista_convenios_responsable.html",
                {
                    "pagina_curso": objeto_pagina_curso,
                    "pagina_suspendido": objeto_pagina_suspendido,
                    "pagina_cancelado": objeto_pagina_cancelado
        ...
        return super(
            VistaListaConveniosResponsable,
            self
        ).get(request)

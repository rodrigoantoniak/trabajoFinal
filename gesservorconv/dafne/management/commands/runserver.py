...
class Command(RunserverCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            default=True,
            help=(
                "Run the old WSGI-based runserver"
                " rather than the ASGI-based one"
            ),
        )
        parser.add_argument(
            "--http_timeout",
    ...
    def inner_run(self, *args, **options):
        ...
        self.stdout.write(
            (
                ...
                "Starting ASGI/Daphne version %(daphne_version)s"
                " development server at %(protocol)s://%(addr)s:%(port)s/\n"
        ...
        logger.debug(
            "Daphne running, listening on %s:%s",
            self.addr, self.port
        )
    ...
    def get_application(self, options):
        """
        Returns the static files serving application
         wrapping the default application,
        if static files should be served.
        Otherwise just returns the default handler.
        """

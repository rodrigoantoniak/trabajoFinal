...
class VistaListaServiciosResponsable(
    MixinAccesoRequerido,
    MixinPermisoRequerido,
    UserPassesTestMixin,
    TemplateView
    ...
    paginate_by: int = 10
    template_name: str = "servicios/listar_servicios_responsable.html"
    ...
    def get_context_data(
        ...
        **kwargs: Dict[str, Any]
        ...
        paginador_curso: Paginator = Paginator(
            servicios.filter(
                Q(completado=False) & Q(cancelacion_servicio__isnull=True)
            ),
            self.paginate_by
        )
        objeto_pagina_curso: Page = paginador_curso.get_page(None)
        contexto["pagina_curso"] = objeto_pagina_curso
        paginador_completado: Paginator = Paginator(
            servicios.filter(
                Q(completado=True) & Q(cancelacion_servicio__isnull=True)
            ),
            self.paginate_by
        )
        objeto_pagina_completado: Page = paginador_completado.get_page(None)
        contexto["pagina_completado"] = objeto_pagina_completado
    ...
    def get(self: Self, request: HtmxHttpRequest) -> HttpResponse:
        if request.htmx:
            ...
            if not tipo or tipo == "completado":
                servicios_completado: QuerySet[Servicio] = servicios.filter(
                    Q(completado=True) & Q(cancelacion_servicio__isnull=True)
                )
                paginador_completado: Paginator = Paginator(
                    servicios_completado,
                    por_pagina if por_pagina != 0 else self.paginate_by
                )
                '''
                Si no existe "pagina" como parámetro del método
                GET, objeto_pagina será 1 por defecto
                '''
                objeto_pagina_completado: Page = paginador_completado.get_page(
                    request.GET.get("pagina")
                )
                if tipo == "completado":
                    return render(
                        request,
                        "parciales/_lista_servicios_completados_responsable.html",
                        {"pagina_completado": objeto_pagina_completado}
                    )
        ...
        return super(
            VistaListaServiciosResponsable,
            self
        ).get(request)

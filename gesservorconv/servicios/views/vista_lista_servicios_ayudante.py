...
class VistaListaServiciosAyudante(
    MixinAccesoRequerido,
    MixinPermisoRequerido,
    UserPassesTestMixin,
    TemplateView
    ...
    paginate_by: int = 10
    template_name: str = "servicios/listar_servicios_ayudante.html"
    ...
    def get_context_data(
        ...
        **kwargs: Dict[str, Any]
        ...
        paginador_curso: Paginator = Paginator(
            servicios.filter(
                Q(pagado=False) & Q(cancelacion_servicio__isnull=True)
            ),
            self.paginate_by
        )
        objeto_pagina_curso: Page = paginador_curso.get_page(None)
        contexto["pagina_curso"] = objeto_pagina_curso
        paginador_pagado: Paginator = Paginator(
            servicios.filter(
                Q(pagado=True) & Q(cancelacion_servicio__isnull=True)
            ),
            self.paginate_by
        )
        objeto_pagina_pagado: Page = paginador_pagado.get_page(None)
        contexto["pagina_pagado"] = objeto_pagina_pagado
    ...
    def get(self: Self, request: HtmxHttpRequest) -> HttpResponse:
        if request.htmx:
            ...
            if not tipo or tipo == "pagado":
                servicios_pagado: QuerySet[Servicio] = servicios.filter(
                    Q(pagado=True) & Q(cancelacion_servicio__isnull=True)
                )
                paginador_pagado: Paginator = Paginator(
                    servicios_pagado,
                ...
                GET, objeto_pagina será 1 por defecto
                '''
                objeto_pagina_pagado: Page = paginador_pagado.get_page(
                    request.GET.get("pagina")
                )
                if tipo == "pagado":
                    return render(
                        request,
                        "parciales/_lista_servicios_pagados_responsable.html",
                        {"pagina_pagado": objeto_pagina_pagado}
                    )

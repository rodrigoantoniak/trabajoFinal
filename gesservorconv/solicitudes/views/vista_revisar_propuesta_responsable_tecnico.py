...
class VistaRevisarPropuestaResponsableTecnico(
    MixinAccesoRequerido,
    TemplateView
):
    template_name: str = "solicitudes/revisar_propuesta_responsable_tecnico.html"
    ...
    def post(self: Self, request: HttpRequest, solicitud: int) -> HttpResponse:
        ...
        decision_responsable_tecnico_propuesta: DecisionResponsableTecnicoPropuesta = \
            DecisionResponsableTecnicoPropuesta.objects.get(
                Q(propuesta_compromisos=propuesta_compromisos),
                Q(responsable_solicitud__responsable_tecnico__usuario_responsable=request.user)
            )
        decision_propuesta: Optional[str] = request.POST.get(
            "decision_propuesta"
        )
        if decision_propuesta:
            decision_responsable_tecnico_propuesta.tiempo_decision_propuesta = TransactionNow()
            if decision_propuesta != "aceptar":
                ...
                decision_responsable_tecnico_propuesta.aceptacion_propuesta = True
                if not (
            ...
            messages.success(
                request,
                "Ha aceptado la propuesta de compromisos"
            )

...
class VistaRechazarSolicitudResponsable(
    MixinAccesoRequerido,
    TemplateView
):
    template_name: str = "solicitudes/rechazar_solicitud_responsable.html"
    ...
    def get(self: Self, request: HttpRequest, solicitud: int) -> HttpResponse:
        ...
        if not ResponsableSolicitud.objects.filter(
            ...
            messages.error(
                "Usted no es Responsable Técnico en este servicio"
            )
            return HttpResponseRedirect(
                reverse_lazy("solicitudes:lista_solicitudes_responsable")
            )
    ...
    def post(self: Self, request: HttpRequest, solicitud: int) -> HttpResponse:
        ...
        messages.success(
            request,
            "Ha rechazado ser Responsable Técnico en la solicitud de servicio"
            " definitivamente"
        )
        return HttpResponseRedirect(
            reverse_lazy("solicitudes:lista_solicitudes_responsable")

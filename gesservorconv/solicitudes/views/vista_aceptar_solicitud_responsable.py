...
class VistaAceptarSolicitudResponsable(
    MixinAccesoRequerido,
    TemplateView
):
    template_name: str = "solicitudes/aceptar_solicitud_responsable.html"
    ...
    def get(self: Self, request: HttpRequest, solicitud: int) -> HttpResponse:
        ...
        if not ResponsableSolicitud.objects.filter(
            ...
            (
                Q(tiempo_decision_comitente__isnull=False) |
                Q(aceptacion_comitente=True)
            )
        ).exists():
            messages.error(
                "Usted no fue invitado como Responsable Técnico en este servicio"
            )
            return HttpResponseRedirect(
                reverse_lazy("solicitudes:lista_solicitudes_responsable")
            )
        ...
        contexto["responsable_solicitud"] = ResponsableSolicitud.objects.get(
            Q(responsable_tecnico__usuario_responsable=request.user) &
            Q(solicitud_servicio__id_solicitud=solicitud)
        )
        return render(
    ...
    def post(self: Self, request: HttpRequest, solicitud: int) -> HttpResponse:
        ...
        responsable_solicitud: ResponsableSolicitud = \
            ResponsableSolicitud.objects.get(
                Q(responsable_tecnico__usuario_responsable=request.user) &
                Q(solicitud_servicio__id_solicitud=solicitud)
            )
        ...
        responsable_solicitud.tiempo_decision_responsable = TransactionNow()
        responsable_solicitud.aceptacion_responsable = True
        with transaction.atomic():
            responsable_solicitud.save()
            solicitud_servicio.save()
        messages.success(
            request,
            "Ha aceptado ser Responsable Técnico en la solicitud de servicio"
            " correctamente"
        )
        return HttpResponseRedirect(
            reverse_lazy("solicitudes:lista_solicitudes_responsable")
